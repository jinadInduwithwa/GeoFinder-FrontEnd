// AmericasCountrySection.jsx
import React, { useState, useEffect } from 'react';
import Marquee from 'react-fast-marquee';
import { motion } from 'framer-motion';
import HomeCard from '../UI/HomeCard';

function AmericasCountrySection() {
  const [countries, setCountries] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchAmericasCountries = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch American countries');
        }
        const data = await response.json();
        console.log('Fetched Americas countries:', data);
        setCountries(data);
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 mb-8 sm:mb-12">
        ...
        >
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-gray-900 mb-4">
            Discover American Countries
          </h2>
          <p className="text-gray-600 text-center max-w-2xl mx-auto text-sm sm:text-base">
            Explore the vibrant cultures, capitals, and landscapes of the Americas' top countries
          </p>
        </motion.div>
      </div>
...
export default AmericasCountrySection;